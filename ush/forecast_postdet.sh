...
EOF
  ...
  if [[ ${ICO2} -gt 0 ]]; then
#ss   for file in $(ls "${FIX_DIR}/am/fix_co2_proj/global_co2historicaldata"*) ; do
...
CICE_postdet() {
  ...
  if [[ "${RUN}" =~ "gfs" || "${RUN}" =~ "gefs" ]]; then
    ...
    for fhr in ${FV3_OUTPUT_FH}; do
      ...
      vdatestr="${vdate:0:4}-${vdate:4:2}-${vdate:6:2}"
      vdatestr2="${vdate:0:4}-${vdate:4:2}"
    ...
    #ss    ${NLN} "${COM_ICE_HISTORY}/ice${vdate}.${ENSMEM}.${current_cycle}.nc" "${DATA}/CICE_OUTPUT/iceh_$(printf "%0.2d" "${interval}")h.${vdatestr}.nc"
        ${NLN} "${COM_ICE_HISTORY}/ice${vdate}.${ENSMEM}.${current_cycle}.nc" "${DATA}/CICE_OUTPUT/iceh.${vdatestr}.nc"
        ${NLN} "${COM_ICE_HISTORY}/iceh.${vdatestr2}.nc" "${DATA}/CICE_OUTPUT/iceh.${vdatestr2}.nc"
